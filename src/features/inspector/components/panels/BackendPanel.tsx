...
export function BackendPanel({ node }: { node: INode }) {
  const updateNodeConfig = useJourneyStore((s) => s.updateNodeConfig)

  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs font-medium text-neutral-700 mb-1">Integration Title</label>
        <input
          type="text"
      ...
      <div className="pt-4 border-t border-neutral-100">
        <p className="text-xs text-neutral-500">
          API parameters and response mapping will be available here.
        </p>
      </div>
    </div>
  )
